...
cCeresDataFile::cCeresDataFile()
	: mImuFrameCounter(0), mLidarFrameCounter(0)
{
    mHasConfigParameters = false;
    mHasSensorInfo = false;
    mHasTimeStamp = false;
    mHasSyncPulseIn = false;
    mHasSyncPulseOut = false;
    mHasMultipurposeIo = false;
    mHasNmea = false;
    mHasTimeInfo = false;
    mHasBeamIntrinsics = false;
    mHasImuIntrinsics = false;
    mHasLidarIntrinsics = false;
    mHasLidarDataFormat = false;
...
/*
pvt::eTIME_UNITS cOusterDataFile::timeUnit() const
{
    if (mHasFrameTimestamp)
        return pvt::eTIME_UNITS::NANOSECONDS;

    return cPvtParser::timeUnit(); 
};

std::uint64_t cOusterDataFile::timeStamp() const
{
    if (mHasFrameTimestamp)
        return getLidarData().timestamp_ns();

    return cPvtParser::timeStamp();
};
*/

/*
bool cOusterDataFile::updateData()
{
    auto oldFrameCount = mLidarFrameCounter;

    while (oldFrameCount == mLidarFrameCounter)
    {
        if (!mFileReader.processBlock())
        {
            return false;
...
void cCeresDataFile::processConfigParam_2(cDataBuffer& buffer)
{
    try
    {
        cOusterParser::processConfigParam_2(buffer);
        mHasConfigParameters = true;
    }
...
void cCeresDataFile::processSensorInfo_2(cDataBuffer& buffer)
{
    try
    {
    ...
    if (cCeresDataFile.product_line.empty())
    {
        ...
        mSensorInfo.base_part_number = "";
        mSensorInfo.base_serial_number = "";

    }
    mHasSensorInfo = true;
}

void cOusterDataFile::processTimestamp_2(cDataBuffer& buffer)
{
    try
    {
...
void cOusterDataFile::processSyncPulseIn_2(cDataBuffer& buffer)
{
    try
    {
        cOusterParser::processSyncPulseIn_2(buffer);
        mHasSyncPulseIn = true;
    }
    catch (const std::exception&)
    {
    }
...
void cOusterDataFile::processSyncPulseOut_2(cDataBuffer& buffer)
{
    try
    {
        cOusterParser::processSyncPulseOut_2(buffer);
        mHasSyncPulseOut = true;
    }
    catch (const std::exception&)
    {
    }
...
void cOusterDataFile::processMultipurposeIO_2(cDataBuffer& buffer)
{
    try
    {
        cOusterParser::processMultipurposeIO_2(buffer);
        mHasMultipurposeIo = true;
    }
    catch (const std::exception&)
    {
    }
...
void cOusterDataFile::processNmea_2(cDataBuffer& buffer)
{
    try
    {
...
void cOusterDataFile::processTimeInfo_2(cDataBuffer& buffer)
{
    try
    {
...
void cOusterDataFile::processBeamIntrinsics_2(cDataBuffer& buffer)
{
    try
    {
    ...
    if (mBeamIntrinsics.altitude_angles_deg.empty())
    {
        mBeamIntrinsics.altitude_angles_deg.resize(128);
        mBeamIntrinsics.altitude_angles_deg = { 46.25, 45.24, 44.54, 44.12,
        ...
        mBeamIntrinsics.azimuth_angles_deg.resize(128);
        mBeamIntrinsics.azimuth_angles_deg = { 11.59, 4.26, -2.95, -10.05, 11.09,
    ...
    mHasBeamIntrinsics = true;
}

void cOusterDataFile::processImuIntrinsics_2(cDataBuffer& buffer)
{
    try
    {
    ...
    mHasImuIntrinsics = true;
}

void cOusterDataFile::processLidarIntrinsics_2(cDataBuffer& buffer)
{
    try
    {
    ...
    mHasLidarIntrinsics = true;
}

void cOusterDataFile::processLidarDataFormat_2(cDataBuffer& buffer)
{
    try
    {
    ...
    if (mLidarDataFormat.pixels_per_column == 0)
    {
        ...
        mLidarDataFormat.column_window_min = 384;
        mLidarDataFormat.column_window_max = 640;
        mLidarDataFormat.udp_profile_lidar = "LEGACY";
        mLidarDataFormat.udp_profile_imu = "LEGACY";
    }

    mHasLidarDataFormat = true;
}

void cOusterDataFile::processImuData(cDataBuffer& buffer)
{
    cOusterParser::processImuData(buffer);
    ++mImuFrameCounter;
}
void cOusterDataFile::processLidarData(cDataBuffer& buffer)
{
    cOusterParser::processLidarData(buffer);
    ++mLidarFrameCounter;
}

void cOusterDataFile::processLidarDataFrameTimestamp(cDataBuffer& buffer)
{
    cOusterParser::processLidarDataFrameTimestamp(buffer);
    mLidarFrameCounter = getLidarData().frame_id();
