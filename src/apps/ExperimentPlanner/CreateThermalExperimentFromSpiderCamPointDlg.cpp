...
cCreateThermalExperimentFromSpiderCamDlg::cCreateThermalExperimentFromSpiderCamDlg(QWidget* parent)
:
...
cCreateThermalExperimentFromSpiderCamDlg::~cCreateThermalExperimentFromSpiderCamDlg()
{}

void cCreateThermalExperimentFromSpiderCamDlg::createControls_PointSelection()
{
	...
	connect(mpScanUnits, &QComboBox::currentTextChanged, this, &cCreateThermalExperimentFromSpiderCamDlg::onScanUnitChange);

	mpSampleXY = new QPushButton("Record X, Y", this);
	mpSampleXY->setEnabled(false);
	connect(mpSampleXY, &QPushButton::pressed, this, &cCreateThermalExperimentFromSpiderCamDlg::recordXY);
}

void cCreateThermalExperimentFromSpiderCamDlg::createLayout_PointSelection(QVBoxLayout* pMainLayout)
{
...
void cCreateThermalExperimentFromSpiderCamDlg::onScanUnitChange(const QString& text)
{
...
bool cCreateThermalExperimentFromSpiderCamDlg::generate()
{
	std::string str;
	QString text;
	...
	for (int scan = 0; scan < numOfScans; ++scan)
	{
		...
		pInfo->setExperimentType(eExperimentType::THERMAL);
...
void cCreateThermalExperimentFromSpiderCamDlg::onShowPath()
{
	int x1_mm = mpStartX_mm->text().toInt();
	int y1_mm = mpStartY_mm->text().toInt();
...
void cCreateThermalExperimentFromSpiderCamDlg::positionUpdated(spidercam::sPosition_1_t pos)
{
