...
cRawDataBuffer::cRawDataBuffer()
	: mpBuffer(nullptr), mWriteIndex(0), mCapacity(0), mOverrun(false)
{
}

cRawDataBuffer::cRawDataBuffer(size_type capacity)
	: mpBuffer(nullptr), mWriteIndex(0), mCapacity(0), mOverrun(false)
{
...
cRawDataBuffer::cRawDataBuffer(const cRawDataBuffer& objToCopy)
	: mpBuffer(nullptr),
...
cRawDataBuffer::cRawDataBuffer(cRawDataBuffer&& objToMove) noexcept
{
...
void cRawDataBuffer::operator=(const cRawDataBuffer& objToCopy)
{
	delete[] mpBuffer;
	mpBuffer = nullptr;
	mpBuffer = new std::byte[objToCopy.mCapacity];
...
std::size_t cRawDataBuffer::write_size() const
{
	if (mWriteIndex > mCapacity) return 0;
	return mCapacity - mWriteIndex;
}

std::size_t cRawDataBuffer::size() const
{
	return mWriteIndex;
}

std::size_t cRawDataBuffer::capacity() const
{
	return mCapacity;
}

void cRawDataBuffer::capacity(size_type capacity)
{
	if (mpBuffer == nullptr)
	{
		mpBuffer = new std::byte[capacity];
...
bool cRawDataBuffer::empty() const
{
	return (mpBuffer == nullptr) || (mWriteIndex == 0);
}

bool cRawDataBuffer::overrun() const
{
	return mOverrun;
}

const std::byte* cRawDataBuffer::data() const
{
	return mpBuffer;
}

void cRawDataBuffer::attach(std::byte* buffer, size_type size)
{
	delete[] mpBuffer;
...
std::byte* cRawDataBuffer::detach()
{
...
void cRawDataBuffer::reset()
{
	mWriteIndex = 0;
	mOverrun = false;
}

void cRawDataBuffer::clear()
{
	reset();
...
std::size_t cRawDataBuffer::insert(const unsigned char* buffer, size_type size)
{
	if (size > write_size())
	{
		mOverrun = true;
...
void cRawDataBuffer::padTo(size_type size, std::byte value)
{
	if (size > mWriteIndex)
	{
...
std::size_t cRawDataBuffer::erase(std::size_t pos, size_type size)
{
...
cRawDataBuffer& cRawDataBuffer::operator<<(const cRawDataBuffer& in)
{
	// Check to make sure we have enough buffer space to put this variable into
	// our internal storage.
...
cRawDataBuffer& cRawDataBuffer::operator<<(const bool in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const char in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const int8_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const uint8_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const int16_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const uint16_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const int32_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const uint32_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const int64_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const uint64_t in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const float in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const double in)
{
	mWriteIndex += ::write(in, wrtbuf(), end(), mOverrun);
	return *this;
}

cRawDataBuffer& cRawDataBuffer::operator<<(const std::string& in)
{
...
/*
void cRawDataBuffer::write(const std::string& in)
{
	if (in.empty())
	{
		return;
...
void cRawDataBuffer::write(const std::byte* in, unsigned char len)
{
	// Check to make sure we have enough buffer space to put this variable into
	// our internal storage.
	if (write_size() < len)
