...
public:
    cDataFile();
	explicit cDataFile(const std::string& filename);

	cDataFile(const cDataFile&) = delete;
	cDataFile(cDataFile&&) = delete;

	cDataFile& operator=(const cDataFile&) = delete;
	cDataFile& operator=(cDataFile&&) = delete;
	...
	void writeBlock(BlockIDs id);
	void writeBlock(BlockIDs id, const std::byte* buf, std::size_t len);
