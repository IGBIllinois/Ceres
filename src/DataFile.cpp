...
cDataFile::cDataFile() : mpFile(nullptr)
{
...
void cDataFile::open(const std::string& filename)
{
...
void cDataFile::writeBlock(BlockIDs id)
{
...
void cDataFile::writeBlock(BlockIDs id, const std::byte* buf, std::size_t len)
{
    if (len == 0)
    {
        writeBlock(id);
