...
cSensorPropertyPage* gps::create_sensor_property_page(const std::string& sensor_id, const std::string& instance,
    const std::string& model, uint32_t version,
    const std::string& remote_ip_address, uint16_t port, const std::string& local_ip_address)
{
    if (sensor_id.compare(ssnx_id) == 0)
    {
        return ssnx::create_sensor_property_page(instance, model, version, remote_ip_address, port, local_ip_address);
    }
