
#include "LucidVisionLabsRgbExperimentStates_Local.hpp"
#include "LucidVisionLabsRgbModel.hpp"
#include "StringUtils.hpp"

#include <QMessageBox>
#include <QThread>

#include <chrono>
#include <thread>


/***************************************************************************/
/**  Base Class for Lucid Vision Labs RGB Camera Local Experiment States  **/
/***************************************************************************/
cLucidVisionLabsRgbCameraExperimentState_Local::cLucidVisionLabsRgbCameraExperimentState_Local(cLucidVisionLabsRgbModel* pModel, QObject* parent)
	: mpModel(pModel)
{}

cLucidVisionLabsRgbCameraExperimentState_Local::~cLucidVisionLabsRgbCameraExperimentState_Local()
{
}

bool cLucidVisionLabsRgbCameraExperimentState_Local::initialize()
{
	return true;
}

void cLucidVisionLabsRgbCameraExperimentState_Local::cleanup()
{}

/***********************************************************************/
/**    Lucid Vision Labs Experiment States to Configure Camera        **/
/***********************************************************************/

cLucidVisionLabsRgbCamera_Configure_Local::cLucidVisionLabsRgbCamera_Configure_Local(cLucidVisionLabsRgbModel* pModel, QObject* parent)
	: cLucidVisionLabsRgbCameraExperimentState_Local(pModel, parent)
{
}

QString cLucidVisionLabsRgbCamera_Configure_Local::getStatusStr()
{
	return "Configuring Lucid Vision Labs camera...";
}

bool cLucidVisionLabsRgbCamera_Configure_Local::configure(const nlohmann::json& stateDoc)
{
	using namespace nlohmann;

	try
	{
		if (stateDoc.contains("mode"))
		{
			auto mode = stateDoc["mode"];
			if (nStringUtils::iequal(mode, "photo"))
			{
				mMode = static_cast<int>(cLucidVisionLabsRgbModel::eMode::SINGLE);
			}
			else if (nStringUtils::iequal(mode, "time lapse") || nStringUtils::iequal(mode, "time-lapse"))
			{
				mMode = static_cast<int>(cLucidVisionLabsRgbModel::eMode::TIME_LAPSE);
			}
			else if (nStringUtils::iequal(mode, "video") || nStringUtils::iequal(mode, "continuous"))
			{
				mMode = static_cast<int>(cLucidVisionLabsRgbModel::eMode::CONTINUOUS);
			}

			mWaitingForMode = mMode != static_cast<int>(mpModel->mode());
		}

		if (stateDoc.contains("frame rate (hz)"))
		{
			mFrameRate_fps = stateDoc["frame rate (hz)"].get<double>();

			mWaitingForFrameRate = mFrameRate_fps != mpModel->frameRate_Hz();
		}

		int32_t lapse_interval_ms = -1;

		if (stateDoc.contains("time lapse interval (s)"))
			lapse_interval_ms = static_cast<int32_t>(stateDoc["time lapse interval (s)"].get<float>() * 1000.0);
		else if (stateDoc.contains("time lapse interval (ms)"))
			lapse_interval_ms = stateDoc["time lapse interval (ms)"].get<int32_t>();

		else if (stateDoc.contains("time-lapse interval (s)"))
			lapse_interval_ms = static_cast<int32_t>(stateDoc["time-lapse interval (s)"].get<float>() * 1000.0);
		else if (stateDoc.contains("time-lapse interval (ms)"))
			lapse_interval_ms = stateDoc["time-lapse interval (ms)"].get<int32_t>();

		else if (stateDoc.contains("lapse interval (s)"))
			lapse_interval_ms = static_cast<int32_t>(stateDoc["lapse interval (s)"].get<float>() * 1000.0);
		else if (stateDoc.contains("lapse interval (ms)"))
			lapse_interval_ms = stateDoc["lapse interval (ms)"].get<int32_t>();

		else if (stateDoc.contains("interval (s)"))
			lapse_interval_ms = static_cast<int32_t>(stateDoc["interval (s)"].get<float>() * 1000.0);
		else if (stateDoc.contains("interval (ms)"))
			lapse_interval_ms = stateDoc["interval (ms)"].get<int32_t>();

		if (lapse_interval_ms > 0)
		{
			mLapseInterval_ms = lapse_interval_ms;

			mWaitingForInterval = mLapseInterval_ms != mpModel->lapseInterval_ms();
		}
	}
	catch (const detail::parse_error& e)
	{
		QString msg = "Parse Error: ";
		msg += e.what();

		QMessageBox mb(QMessageBox::Critical, "Lucid Vision Labs RGB Experiment State Error", msg);
		mb.exec();

		return false;
	}
	catch (const detail::type_error& e)
	{
		QString msg = "Type Error: ";
		msg += e.what();

		QMessageBox mb(QMessageBox::Critical, "Lucid Vision Labs RGB Experiment State Error", msg);
		mb.exec();

		return false;
	}
	catch (const detail::exception& e)
	{
		QString msg = "Unknown Error: ";
		msg += e.what();

		QMessageBox mb(QMessageBox::Critical, "Lucid Vision Labs RGB Experiment State Error", msg);
		mb.exec();

		return false;
	}

	return true;
}

void cLucidVisionLabsRgbCamera_Configure_Local::run()
{
	if (mUpdateConfiguration)
	{
		if (mWaitingForMode)
			emit requestMode(mMode);

		if (mWaitingForFrameRate)
			emit requestFrameRate_Hz(mFrameRate_fps);

		if (mWaitingForInterval)
			emit requestLapseInterval_ms(mLapseInterval_ms);

		mUpdateConfiguration = false;
	}
}

void cLucidVisionLabsRgbCamera_Configure_Local::pause() {}
void cLucidVisionLabsRgbCamera_Configure_Local::stop() {}

cExperimentState::eRESULT cLucidVisionLabsRgbCamera_Configure_Local::finished()
{
	if (mWaitingForMode || mWaitingForFrameRate || mWaitingForInterval)
		return cExperimentState::eRESULT::WAITING;

	return cExperimentState::eRESULT::DONE;
}

void cLucidVisionLabsRgbCamera_Configure_Local::modeChanged(int mode)
{
	mWaitingForMode = false;
}

void cLucidVisionLabsRgbCamera_Configure_Local::lapseIntervalChanged(int interval_ms)
{
	mWaitingForInterval = false;
}

void cLucidVisionLabsRgbCamera_Configure_Local::frameRateChanged(double rate_fps)
{
	mWaitingForFrameRate = false;
}


/***********************************************************************/
/**       Lucid Vision Labs Experiment States to Take Photo           **/
/***********************************************************************/

cLucidVisionLabsRgbCamera_TakePhoto_Local::cLucidVisionLabsRgbCamera_TakePhoto_Local(cLucidVisionLabsRgbModel* pModel, QObject* parent)
	: cLucidVisionLabsRgbCameraExperimentState_Local(pModel, parent)
{
}

QString cLucidVisionLabsRgbCamera_TakePhoto_Local::getStatusStr()
{
	if (mUpdateView)
		return "Taking Photo and updating view...";

	return "Taking Photo...";
}

bool cLucidVisionLabsRgbCamera_TakePhoto_Local::configure(const nlohmann::json& stateDoc)
{
	using namespace nlohmann;

	mUpdateView = false;

	try
	{
		if (stateDoc.contains("update view"))
		{
			mUpdateView = stateDoc["update view"];
		}
	}
	catch (const detail::parse_error& e)
	{
		QString msg = "Parse Error: ";
		msg += e.what();

		QMessageBox mb(QMessageBox::Critical, "Lucid Vision Labs RGB Experiment State Error", msg);
		mb.exec();

		return false;
	}
	catch (const detail::type_error& e)
	{
		QString msg = "Type Error: ";
		msg += e.what();

		QMessageBox mb(QMessageBox::Critical, "Lucid Vision Labs RGB Experiment State Error", msg);
		mb.exec();

		return false;
	}
	catch (const detail::exception& e)
	{
		QString msg = "Unknown Error: ";
		msg += e.what();

		QMessageBox mb(QMessageBox::Critical, "Lucid Vision Labs RGB Experiment State Error", msg);
		mb.exec();

		return false;
	}

	return true;
}

void cLucidVisionLabsRgbCamera_TakePhoto_Local::run()
{
	if (mTriggerPhoto)
	{
		if (mpModel->mode() == cLucidVisionLabsRgbModel::eMode::SINGLE)
		{
			emit takePhoto(mUpdateView);
		}
		else
		{
			if (mUpdateView)
				emit updateView();

			mResult = cExperimentState::eRESULT::DONE;
		}

		mTriggerPhoto = false;
	}
}

void cLucidVisionLabsRgbCamera_TakePhoto_Local::pause() {}
void cLucidVisionLabsRgbCamera_TakePhoto_Local::stop() {}

cExperimentState::eRESULT cLucidVisionLabsRgbCamera_TakePhoto_Local::finished()
{
	return mResult;
}

void cLucidVisionLabsRgbCamera_TakePhoto_Local::onPhotoTaken()
{
	mResult = cExperimentState::eRESULT::DONE;
};



