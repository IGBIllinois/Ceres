...
namespace hyspex
{
    ...
    */
    ...
    public:
        ...
        HYSPEXLIBRARY_API ReturnCode stopMovingAverageBackgroundCalculation( unsigned int a_timeoutMs = 0 ); //!< Stop moving average calculation and wait until timeout.
        ...
        HYSPEXLIBRARY_API ReturnCode stopCalculatingBackground(); //!< Stops calculating background, and stores result.
        ...
        // Mjolnir Specific Auto Acquisition functions
        HYSPEXLIBRARY_API ReturnCode setAutoAcquisitionMode( AutoAcquisitionMode a_mode  ); //!< Mjolnir only: set auto acquisition mode.
        HYSPEXLIBRARY_API ReturnCode getAutoAcquisitionMode( AutoAcquisitionMode* a_mode ); //!< Mjolnir only: get auto acquisition mode.
        HYSPEXLIBRARY_API ReturnCode getAutoAcquisitionState( bool* a_state ); //!< Newer Mjolnir only: get auto acquisition state
        ...
        HYSPEXLIBRARY_API const ConstBuffer< double >& getREMatrix() const; //!< Get RE matrix from settings file
        HYSPEXLIBRARY_API const ConstBuffer< double >& getQEMatrix() const; //!< Get Quantum Efficiency (QE) matrix from settings file

        HYSPEXLIBRARY_API const ConstBuffer< double >& getSpectralCalibrationMatrix() const; //!< Deprecated, same as getSpectralCalibrationPerBand()
        HYSPEXLIBRARY_API const ConstBuffer< double >& getFullSpectralCalibrationMatrix() const; //!< Deprecated, same as getFullSpectralCalibrationPerBand()
        HYSPEXLIBRARY_API const ConstBuffer< double >& getSpectralDeltaMatrix() const; //!< Deprecated, same as getSpectralDeltaPerBand()

        HYSPEXLIBRARY_API const ConstBuffer< double >& getSpectralCalibrationPerBand() const; //!< Get calibrated spectral wavelength in nm for each spectral band.
        HYSPEXLIBRARY_API const ConstBuffer< double >& getFullSpectralCalibrationPerBand() const; //!< Get calibrated spectral wave length in nm for each spectral band. (full calibrated sensor)
        HYSPEXLIBRARY_API const ConstBuffer< double >& getSpectralDeltaPerBand() const; //!< Get delta wavelength in nm for each spectral band.

        HYSPEXLIBRARY_API const ConstBuffer< double >& getSpectralCalibrationPerPixel() const; //!< Get calibrated spectral wavelength in nm for each spectral pixel. NB: just duplicated from spectral_calib unless spectral_calib_matrix is in .set file.
        HYSPEXLIBRARY_API const ConstBuffer< double >& getFullSpectralCalibrationPerPixel() const; //!< Get calibrated spectral wave length in nm for each pixel. (full calibrated sensor) NB: just duplicated from spectral_calib unless spectral_calib_matrix is in .set file.
        HYSPEXLIBRARY_API const ConstBuffer< double >& getSpectralDeltaPerPixel() const; //!< Get delta wavelength in nm for each pixel NB: just duplicated from spectral_calib unless spectral_calib_matrix is in .set file.
