...
namespace hyspex
{
    ...
    */
    ...
    /*! Auto Acquisition Mode (Mjolnir only)
     *
    ...
    */
    ...
    /*!
    * HySpex header file revision.
    * The HySpex file format is as follows:
    * Version 0: [ HYSPEX\0 (8B) ][ image_data ( int (4B) ) ][ binary struct ] 
    * Version 1: [ HYSPEX (6B) ][ file revision (unsigned int (4B)) ]
    * Version 2: [ HYSPEX (6B) ][ file revision (unsigned int (4B)) ][ header size (unsigned int (4B)) ]
    * [ size of property string ( unsigned long (8B ) ) ][ property string (...) ]
    *
    * There are currently 3 different top level properties:
    * "properties"
    * "arrays"
    * "image_data"
    * any other top level property will be ignored, and will cause a read failure.
    *
    * "properties" are assigned as follows:
    * [ size of count string ( unsigned long (8B) ) ][ count string (...) ]
    * for 0 .. count:
    *   [ size of key string ( unsigned long (8B) ) ][ key string (...) ] [ size of value string ( unsigned long (8B) ) ][ value string (...) ]
    *
    * "arrays" are assigned as follows:
    * [ size of count string ( unsigned long (8B) ) ][ count string (...) ]
    * for 0 .. count:
    *   [ size of array type string (unsigned long (8B) ) ][ array type string (...) ] [ size of array in elements ( unsigned long (8B) ) ][ array data (...) ]
    *
    * currently supported types are "int" and "double".
    *
    * image_data is assigned as follows:
    * [ image count (unsigned long (8B)) ][ image size in elements (unsigned long (8B)) ]
    * for 0 .. count:
    *   [ image_data (image size in elements * 2B) ]
    *
    * History:
    * 0 - legacy format, almost all current recordings are in this format.
    * 1 - first version, expect changes. This format might not be readable in the future.
    * 2 - current version
    */
    static const uint32_t HYSPEX_FILE_HEADER_VERSION = 2;
}
